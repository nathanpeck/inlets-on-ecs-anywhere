...
export class InletsStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    // Generate a token that will be used by clients to connect
    // to the exit server.
    ...
    // Launch the Inlets exit server as a daemon in Fargate.
    var exitServerDefinition = new ecs.FargateTaskDefinition(this, 'inlets-exit-server-task', {
      cpu: 1024,
      memoryLimitMiB: 2048,
    });

    exitServerDefinition.addContainer('inlets', {
      ...
      command: ["http", "server", "--auto-tls=false", "--token-env=INLETS_TOKEN"],
      secrets: {
    ...
    // Launch an NLB that will sit in front of the inlet exit server, for HA
    ...
    inletsClientListener.addTargets('inlets', {
      port: 8123,
      targets: [service]
    });
    ...
    inletsPublicListener.addTargets('inlets', {
      port: 8080,
      targets: [service]
    });

    // Create an Anywhere service to run in the cluster
    var clientDefinition = new ecs.ExternalTaskDefinition(this, 'client-inlets');

    clientDefinition.addContainer('inlets', {
      cpu: 256,
      memoryLimitMiB: 256,
      image: ecs.ContainerImage.fromRegistry('ghcr.io/inlets/inlets-pro:0.9.0-rc2'),
      logging: new ecs.AwsLogDriver({ streamPrefix: 'inlets-client' }),
      command: [
        "http", "client", "--url=ws://" + lb.loadBalancerDnsName + ":8123", "--token-env=INLETS_TOKEN", "--upstream", "localhost:8080",
    ...
    // Define the external service that we will expose via Inlets
    ...
    appDefinition.addContainer('app', {
      cpu: 2048,
      memoryLimitMiB: 2048,
      image: image,
    ...
    var appService = new ecs.ExternalService(this, 'demo-app-service', {
      cluster,
